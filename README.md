...
- **Flexible sources** — fourteen (14) deterministic generated sounds, drag-and-drop audio files, microphone/device input, and browser-tab audio where the platform supports it. Source monitoring is off by default so you hear the processed instrument, not the raw generator; microphone and tab sources never monitor through the output, preventing feedback.
...
- **23 authored presets** — frozen, metallic, textural, organ, and living voices spanning all fourteen generated sources, so each preset's character is carried by the generator best suited to it. Selecting a preset is a complete scene: it clears any held A/B snapshots so the preset's own source is heard, while loading a saved session restores that session's snapshots. Presets preselect a capture mode but never auto-capture — capturing stays a manual action.
